...
## 6. App Settings & Biometric Lock
- **Biometric Security:** Integrated `androidx.biometric:biometric` to provide an app-wide lock screen. When enabled in global settings, it locks the app on launch and upon returning to the foreground, requiring strong biometric or device credential authentication.
- **Splash Delay:** Implemented a brief 1.5s splash screen delay in `MainActivity` to smooth out the startup experience before content or the lock screen appears.
- **Dynamic Theming:** Upgraded `Theme.kt` and `MainActivity.kt` to dynamically read the theme preference (Light, Dark, or System) from the `SettingsDataStore`.
- **Global Settings Screen:** Expanded the global settings UI with options for theme selection (via `AlertDialog`), showing video thumbnails, and specifying a custom user agent.
- **Account Settings Screen:** Massively expanded account-specific preferences. Added grouped sections for Privacy, Bucket (MD5, Storage Class), Upload (Multipart settings, concurrency), Thumbnails, Cleanup (Cache clearing, pending upload deletion), and Network (timeouts). Utilized Sliders and numeric input Dialogs for fine-grained tuning.
- **About Screen:** Created a dedicated `AboutScreen` with the application logo, version info fetched from `PackageManager`, a GitHub profile link, and a placeholder PIX QR code with click-to-copy functionality for the clipboard.
...
- Dynamic theme selection applies instantly from the Settings screen.
- Biometric lock properly guards the application on cold starts and foreground returns when enabled.
- Fixed a major issue where cancelling the AppLock prompt or returning from the app switcher would cause the screen to vanish and the app to close.
- Modified the lock screen to use an overlay approach, perfectly preserving the underlying app's navigation state while blocking access.
- Implemented a fallback "Unlock" button on the lock screen so users can quickly retry authentication.
- The build verified successfully with no compilation errors using Gradle on PowerShell.
